...
app.get("/api/users", authenticateToken, (req, res) => {
  db.all("SELECT id, username, email, gender, age, created_at FROM users", [], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ data: rows });
  });
});
...
app.get("/api/users/:id", authenticateToken, (req, res) => {
  const { id } = req.params;
  db.get("SELECT id, username, email, gender, age, created_at FROM users WHERE id = ?", [id], (err, user) => {
    if (err) return res.status(500).json({ error: err.message });
...
app.put("/api/users/:id", authenticateToken, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      if (this.changes === 0) return res.status(404).json({ message: "User không tồn tại" });
