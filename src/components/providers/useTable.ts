...
export function useTable<T>({ fetchData, initialFilter = {} }: TableInterface<T>) {
  const currentPage = ref<number>(1);
  const perPage = ref<number>(DEFAULT_PER_PAGE);
  const filter = ref<Record<string, any>>(initialFilter);

  const queryKey = ['table-data', { ...filter.value }, currentPage.value, perPage.value];

  const { data, isFetching, refetch } = useQuery({
    queryKey,
    queryFn: () => fetchData({ ...filter.value, page: currentPage.value, limit: perPage.value }),
  });
  ...
  return { data, isFetching, setFilter, setPage, setPerPage, currentPage, perPage };
}
